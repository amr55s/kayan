-- Final, machine-verifiable release boundary. Readiness must fail closed when
-- any security, commerce, worker, or branch capability in this release is
-- absent, even if the original foundation migration exists.
begin;
...
values (
  ...
  pg_catalog.jsonb_build_object(
    'version', '20260818181810',
    'name', 'marketplace_release_marker',
    'capabilities', pg_catalog.jsonb_build_array(
      'catalog_search_v2',
      'admin_aal2',
      'partial_returns',
      'reversible_media',
      'granular_admin_roles',
      'store_branches',
      'notifications',
      'spaces_media'
    )
...
begin
  ...
  select pg_catalog.count(distinct proc.proname)::integer
  into v_function_count
  from pg_catalog.pg_proc as proc
  join pg_catalog.pg_namespace as namespace
    on namespace.oid = proc.pronamespace
  where namespace.nspname = 'public'
    and proc.proname = any (v_required_functions);

  if v_function_count <> pg_catalog.array_length(v_required_functions, 1) then
    return false;
  end if;
...
revoke all on function public.marketplace_release_ready()
