...
values (
  ...
  pg_catalog.jsonb_build_object(
    'version', '20260905203103',
    'name', 'verified_onboarding_release_marker',
    'capabilities', pg_catalog.jsonb_build_array(
      ...
      'order_branch_composite_assignment', 'chat_message_retry_idempotency'
    )
  ), null, pg_catalog.now()
)
...
declare
  ...
  v_required_functions constant text[] := array[
    ...
    'assign_marketplace_order_branch', 'send_my_marketplace_chat_message'
  ];
...
begin
  ...
  if v_version is distinct from '20260905203103' then return false; end if;
