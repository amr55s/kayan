...
export default async function SharePage() {
  const [servicesResult, driversResult] = await Promise.allSettled([
    fetchPublicServices({ page: 1 }),
    fetchPublicDriverSummaries(4),
  ]);
  const sharePlaces = (
    servicesResult.status === 'fulfilled' ? servicesResult.value.items : []
  ).slice(0, 6).map(({ id, title, category }) => ({
    id,
    title,
    category,
  }));
      ...
      <main id="main-content">
        <section className="border-b border-zinc-200 bg-zinc-950 px-4 py-12 text-center text-white sm:py-16">
          ...
          <h1 className="mx-auto mt-4 max-w-3xl text-3xl font-black leading-tight sm:text-5xl">
            اختار بطاقة وشاركها بالطريقة المناسبة لك
          </h1>
