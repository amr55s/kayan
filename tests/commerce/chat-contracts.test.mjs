...
test('shared ChatMessage schema requires a positive integer durable revision', () => {
  ...
  assert.equal(sharedChatMessageSchema.safeParse({ ...value, attachment: { id: crypto.randomUUID(), url: 'ftp://private', width: 10, height: 10, alt: '' } }).success, false);
  assert.equal(sharedChatMessageSchema.safeParse({ ...value, body: '😀'.repeat(5001) }).success, false);
  assert.equal(sharedChatMessageSchema.safeParse({ ...value, reactions: [{ emoji: '💣', count: 1, reactedByMe: false }] }).success, false);
