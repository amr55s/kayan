...
export function classifyChatRisk(input: ChatRiskInput): ChatRiskSignal[] {
  ...
  if (phone.test(body) || hasExternalLink(body)) signals.push({ type: 'external_contact', score: 2, evidence: [phone.test(body) ? 'phone_detected' : 'external_link_detected'] });
...
/** Exact host or a dot-delimited first-party subdomain only.  Suffixes such as
 * dairtak.com.attacker.test and lookalike hosts must remain external. */
export function isDairtakHostname(hostname: string): boolean {
  const normalized = hostname.trim().toLowerCase().replace(/\.+$/u, '');
  return normalized === 'dairtak.com' || normalized.endsWith('.dairtak.com');
}

function hasExternalLink(body: string): boolean {
  for (const candidate of body.match(link) ?? []) {
    try { if (!isDairtakHostname(new URL(candidate).hostname)) return true; } catch { return true; }
  }
  return false;
}
